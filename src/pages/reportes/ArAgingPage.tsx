...
type UnifiedArRow = (ArAgingRow & { _isAdvance?: false }) | (any & { _isAdvance: true; _rowKey: string })

const unifiedColumns = [
  {
    title: 'Documento',
    width: 160,
    render: (_: any, row: UnifiedArRow) => row._isAdvance
      ? <span style={{ fontWeight: 600, fontSize: 13, color: '#2ea172' }}>{row.advanceNumber}</span>
      : <Link to={`/ventas/facturas/${(row as ArAgingRow).id}`} style={{ fontWeight: 600, fontSize: 13 }}>{(row as ArAgingRow).invoiceNumber}</Link>,
  },
  {
    title: 'Cliente',
  ...
  },
  {
    ...
    align: 'right' as const,
    render: (_: any, row: UnifiedArRow) => {
      if (row._isAdvance) return <Text type="secondary" style={{ fontSize: 12 }}>—</Text>
      const r = row as ArAgingRow
...
function BucketCard({ label, bucket, color, expanded, onToggle, advances, showAdvances }: BucketCardProps) {
  ...
  const rows: UnifiedArRow[] = [
    ...bucket.items,
    ...visibleAdvances.map((a: any) => ({ ...a, _isAdvance: true as const, _rowKey: `adv-${a.id}` })),
  ]
    ...
    >
      {expanded && rows.length > 0 && (
        <Table<UnifiedArRow>
          ...
          rowKey={(row) => (row as any)._isAdvance ? (row as any)._rowKey : (row as ArAgingRow).id}
          pagination={false}
          size="small"
          style={{ marginTop: 4 }}
          expandable={{
            expandedRowRender: (row: UnifiedArRow) => {
              if (row._isAdvance) return null
              const r = row as ArAgingRow
            ...
            rowExpandable: (row: UnifiedArRow) => !row._isAdvance && ((row as ArAgingRow).paidAmount ?? 0) > 0,
    ...
    <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
      ...
      {data && (
        <>
          ...
          {/* Resumen por cliente */}
          {(() => {
            const byCustomer = new Map<string, { name: string; cxc: number; anticipo: number }>()
            const ensure = (key: string, name: string) => {
              if (!byCustomer.has(key)) byCustomer.set(key, { name, cxc: 0, anticipo: 0 })
              return byCustomer.get(key)!
            }
            for (const bucket of Object.values(data.buckets) as any[]) {
              for (const item of bucket.items ?? []) {
                ensure(item.customerId || '__sin__', item.customerName || '—').cxc += Number(item.balanceGTQ ?? item.balance ?? 0)
              }
            }
            for (const adv of data.advances ?? []) {
              ensure(adv.customerId || '__sin__', adv.customerName || '—').anticipo += Number(adv.balance ?? 0)
            }
            const rows = [...byCustomer.entries()].map(([key, v]) => ({
              key,
              customerName: v.name,
            ...
            return (
              <Table
                ...
                title={() => (
                  <span style={{ fontWeight: 700, fontSize: 13, color: '#1B3A6B' }}>
                    Resumen por cliente
                  </span>
                )}
                summary={() => (
                  <Table.Summary.Row style={{ background: '#f0f4fa', fontWeight: 700 }}>
                    <Table.Summary.Cell index={0}><Text strong>TOTAL GENERAL</Text></Table.Summary.Cell>
                    ...
                    <Table.Summary.Cell index={3} align="right">
                      <Text strong style={{ fontSize: 13, color: totNet <= 0 ? '#2ea172' : '#e5484d' }}>
                ...
                columns={[
                  ...
                  {
                    title: 'Total CxC', dataIndex: 'cxc', width: 160, align: 'right' as const,
                    render: (v: number) => v > 0
                      ? <Text style={{ color: '#1faec2', fontWeight: 600 }}>{fmt(v)}</Text>
                      : <Text type="secondary">—</Text>,
                  },
                  {
                    title: 'Anticipos', dataIndex: 'anticipo', width: 160, align: 'right' as const,
                    render: (v: number) => v > 0
                      ? <Text style={{ color: '#2ea172', fontWeight: 600 }}>({fmt(v)})</Text>
                      : <Text type="secondary">—</Text>,
                  },
                  {
                    title: 'Neto', dataIndex: 'neto', width: 180, align: 'right' as const,
                    render: (v: number) => (
                      <Text style={{ fontWeight: 700, color: v <= 0 ? '#2ea172' : '#e5484d' }}>
                        {v < 0

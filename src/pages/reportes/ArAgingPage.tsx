...
import { getArAging, type ArAgingRow } from '../../api/facturas'
...
interface ClientAgingRow {
  ...
  invoices: Array<ArAgingRow & { bucket: BucketKey }>
}

function buildClientRows(data: any): ClientAgingRow[] {
  const map = new Map<string, ClientAgingRow>()

  const ensure = (cid: string, cname: string): ClientAgingRow => {
    if (!map.has(cid)) {
      map.set(cid, {
  ...
  for (const key of BUCKET_KEYS) {
    for (const item of (data.buckets[key]?.items ?? []) as ArAgingRow[]) {
      const row = ensure(item.customerId || '__sin__', item.customerName || '—')
  ...
  for (const adv of data.advances ?? []) {
    const row = ensure(adv.customerId || '__sin__', adv.customerName || '—')
...
export default function ArAgingPage() {
  ...
  const handleTableChange: TableProps<ClientAgingRow>['onChange'] = (_, __, sorter) => {
  ...
  const expandedRowRender = useCallback((row: ClientAgingRow) => {
    const invCols: ColumnsType<any> = [
      {
        title: 'Documento', width: 140,
        render: (_: any, inv: any) => (
          <Link to={`/ventas/facturas/${inv.id}`} style={{ fontWeight: 600, fontSize: 12 }}>
            {inv.invoiceNumber}
          </Link>
        ),
      },
  ...
  const columns: ColumnsType<ClientAgingRow> = useMemo((): ColumnsType<ClientAgingRow> => [
    {
      title: 'Cliente',
      key: 'customerName',
      width: 260,
      sorter: (a, b) => a.customerName.localeCompare(b.customerName, 'es'),
      ...
      render: (_, row) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <Text strong style={{ fontSize: 13 }}>{row.customerName}</Text>
    ...
    ...BUCKET_KEYS.map((key): ColumnsType<ClientAgingRow>[number] => ({
      title: BUCKET_LABELS[key],
      key,
      dataIndex: key,
      width: 120,
      align: 'right',
      sorter: (a: ClientAgingRow, b: ClientAgingRow) => a[key] - b[key],
  ...
  const handleExcel = () => {
    if (!data) return
    const rows: any[][] = [
      ['AR Aging — Cuentas por Cobrar por Cliente'],
      [`Período: ${periodLabel}`, '', '', '', '', '', '', `Corte: ${data.asOf}`, `Generado: ${new Date(data.generatedAt).toLocaleString('es-GT')}`],
      [],
      ['Cliente', 'Vigente', '1-30 días', '31-60 días', '61-90 días', '+90 días', 'Anticipo', 'Total CxC', 'Neto', '% Vencido'],
    ]
    ...
    return (
      <Table.Summary fixed="bottom">
        <Table.Summary.Row style={{ background: '#e6fafd', fontWeight: 700 }}>
          {/* colSpan=2: cubre la columna del ícono expandir + columna Cliente */}
          <Table.Summary.Cell index={0} colSpan={2}>
            <Text strong style={{ fontSize: 12, color: '#1B3A6B', whiteSpace: 'nowrap' }}>
              TOTAL GENERAL — {clientRows.length} clientes
            </Text>
          </Table.Summary.Cell>
    ...
    <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <Title level={4} style={{ margin: 0, color: '#0a0a0a' }}>AR Aging — Cuentas por Cobrar</Title>
        </div>
        <Space>
      ...
      {data && (
        <>
          ...
          {/* Top 5 clientes con mayor urgencia */}
          {top5.length > 0 && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <Text strong style={{ fontSize: 13, color: '#e5484d' }}>
                  Atención — {top5.length} cliente{top5.length > 1 ? 's' : ''} con saldos vencidos +60 días
                </Text>
              </div>
                ...
                {top5.map(c => (
                  <div key={c.customerId} style={{
                    ...
                    <Text strong style={{ fontSize: 11, display: 'block', color: '#374151', marginBottom: 2 }}>
                      {c.customerName.length > 24 ? c.customerName.slice(0, 24) + '…' : c.customerName}
                    </Text>
                    {c.over_90 > 0 && (
                      <Text style={{ fontSize: 12, color: '#991b1b', display: 'block', fontVariantNumeric: 'tabular-nums' }}>
                        +90d: <strong>{fmt(c.over_90)}</strong>
                      </Text>
                    )}
                    {c.days_90 > 0 && (
                      <Text style={{ fontSize: 12, color: '#e5484d', display: 'block', fontVariantNumeric: 'tabular-nums' }}>
                        61-90d: {fmt(c.days_90)}
                      </Text>
                    )}
                    <Text style={{ fontSize: 10, color: '#6b7280' }}>Total: {fmt(c.total)}</Text>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: 24, textAlign: 'center' }}>
              ...
              <div>
                <Text style={{ color: '#2ea172', fontSize: 15 }}>
                  No hay saldos pendientes de cobro al cierre de {periodLabel}.
                </Text>
              </div>
            </div>
          )}
